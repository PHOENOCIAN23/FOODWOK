...
export const MenuProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      if (res.ok) {
        ...
        if (data && data.lastUpdated && data.lastUpdated > lastServerSyncTimestamp) {
          if (Array.isArray(data.items) && data.items.length > 0) {
            setItems(data.items);
            localStorage.setItem('foodwok_admin_v4_menu_items', JSON.stringify(data.items));
          }
          if (Array.isArray(data.addOns) && data.addOns.length > 0) {
            setAddOns(data.addOns);
            localStorage.setItem('foodwok_admin_v4_addons', JSON.stringify(data.addOns));
          }
          if (Array.isArray(data.categories) && data.categories.length > 0) {
            setCategories(data.categories);
            localStorage.setItem('foodwok_admin_v4_categories', JSON.stringify(data.categories));
          }
          setLastServerSyncTimestamp(data.lastUpdated);
        }
      }
  ...
  const loadSavedData = useCallback(() => {
    const savedItemsRaw = localStorage.getItem('foodwok_admin_v4_menu_items');
    const savedAddOnsRaw = localStorage.getItem('foodwok_admin_v4_addons');
    const savedCategoriesRaw = localStorage.getItem('foodwok_admin_v4_categories');
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key === 'foodwok_admin_v4_menu_items' ||
        e.key === 'foodwok_admin_v4_addons' ||
        e.key === 'foodwok_admin_v4_categories'
      ) {
        loadSavedData();
      }
    };
  ...
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('foodwok_admin_v4_menu_items', JSON.stringify(items));
    }
  }, [items, isMounted]);

  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('foodwok_admin_v4_addons', JSON.stringify(addOns));
    }
  }, [addOns, isMounted]);

  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('foodwok_admin_v4_categories', JSON.stringify(categories));
    }
